...
contract RSP {
    ...
    function hashForChoice(Rsp choice) external pure returns (bytes32) {
        return keccak256(choice);
    }
    
    function submit(Rsp choice) external {
        if ( msg.sender == player1 ) {
            if ( keccak256(choice) == player1ChoiceHash ) {  player1Choice = choice; }
        }
        else if ( msg.sender == player2 ) {
            if ( keccak256(choice) == player2ChoiceHash ) { player2Choice = choice;}
        }
    }
}
